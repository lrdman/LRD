...
import static lrd.user.JSONResponses.LOCK_ACCOUNT;
...
public final class UnlockAccount extends UserServlet.UserRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        if (account == null) {
            ...
            try (DbIterator<? extends Transaction> transactions = Lrd.getTransactionProcessor().getAllUnconfirmedTransactions()) {
                while (transactions.hasNext()) {
                    ...
                    if (Arrays.equals(transaction.getSenderPublicKey(), accountPublicKey)) {
                        ...
                        myTransaction.put("sentAmountLQT", transaction.getAmountLQT());
                        if (accountId == transaction.getRecipientId()) {
                            myTransaction.put("receivedAmountLQT", transaction.getAmountLQT());
                        }
                        myTransaction.put("feeLQT", transaction.getFeeLQT());
            ...
            int blockchainHeight = Lrd.getBlockchain().getLastBlock().getHeight();
            try (DbIterator<? extends Block> blockIterator = Lrd.getBlockchain().getBlocks(account, 0)) {
                while (blockIterator.hasNext()) {
                    Block block = blockIterator.next();
                    if (block.getTotalFeeLQT() > 0) {
                        JSONObject myTransaction = new JSONObject();
                        ...
                        myTransaction.put("earnedAmountLQT", block.getTotalFeeLQT());
            ...
            try (DbIterator<? extends Transaction> transactionIterator = Lrd.getBlockchain().getTransactions(account, (byte) -1, (byte) -1, 0)) {
