    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        try (DbIterator<? extends Transaction> transactions = Lrd.getTransactionProcessor().getAllUnconfirmedTransactions()) {
            while (transactions.hasNext()) {
                ...
                unconfirmedTransaction.put("amountLQT", transaction.getAmountLQT());
                unconfirmedTransaction.put("feeLQT", transaction.getFeeLQT());
        ...
        try (DbIterator<? extends Block> lastBlocks = Lrd.getBlockchain().getBlocks(0, 59)) {
            for (Block block : lastBlocks) {
                ...
                recentBlock.put("totalAmountLQT", block.getTotalAmountLQT());
                recentBlock.put("totalFeeLQT", block.getTotalFeeLQT());
