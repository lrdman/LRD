...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        private final long amountLQT;
        private final long feeLQT;
        ...
        BuilderImpl(byte version, byte[] senderPublicKey, long amountLQT, long feeLQT, short deadline,
                    Attachment.AbstractAttachment attachment) {
            this.version = version;
            this.deadline = deadline;
            this.senderPublicKey = senderPublicKey;
            this.amountLQT = amountLQT;
            this.feeLQT = feeLQT;
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws LrdException.NotValidException {
        ...
        @Override
        public TransactionImpl build() throws LrdException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws LrdException.NotValidException {

        this.timestamp = builder.timestamp;
        this.deadline = builder.deadline;
        this.senderPublicKey = builder.senderPublicKey;
        this.recipientId = builder.recipientId;
        this.amountLQT = builder.amountLQT;
        ...
        if (builder.feeLQT <= 0) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Lrd.getBlockchain().getHeight());
            if (this.phasing == null) {
                feeLQT = getMinimumFeeLQT(effectiveHeight);
            } else {
                feeLQT = Math.max(getMinimumFeeLQT(effectiveHeight), getMinimumFeeLQT(phasing.getFinishHeight()));
            }
        } else {
            feeLQT = builder.feeLQT;
        }

        if (builder.signature != null && secretPhrase != null) {
            throw new LrdException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new LrdException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    byte[] bytes() {
        if (bytes == null) {
            try {
                ...
                if (useLQT()) {
                    buffer.putLong(amountLQT);
                    buffer.putLong(feeLQT);
                    ...
                    buffer.putInt((int) (amountLQT / Constants.ONE_LRD));
                    buffer.putInt((int) (feeLQT / Constants.ONE_LRD));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws LrdException.NotValidException {
        try {
            ...
            long amountLQT = buffer.getLong();
            long feeLQT = buffer.getLong();
            ...
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey, amountLQT, feeLQT,
            ...
            return builder;
        } catch (LrdException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws LrdException.NotValidException {
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("amountLQT", amountLQT);
        json.put("feeLQT", feeLQT);
        if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws LrdException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new LrdException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws LrdException.NotValidException {
        try {
            ...
            long amountLQT = Convert.parseLong(transactionData.get("amountLQT"));
            long feeLQT = Convert.parseLong(transactionData.get("feeLQT"));
            ...
            if (transactionType == null) {
                throw new LrdException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey,
                    amountLQT, feeLQT, deadline,
    ...
    private boolean checkSignature() {
        if (!hasValidSignature) {
            hasValidSignature = signature != null && Crypto.verify(signature, zeroSignature(getBytes()), getSenderPublicKey(), useLQT());
        }
    ...
    private int signatureOffset() {
        return 1 + 1 + 4 + 2 + 32 + 8 + (useLQT() ? 8 + 8 + 32 : 4 + 4 + 8);
    }

    private boolean useLQT() {
        return this.height > Constants.LQT_BLOCK
                && (this.timestamp > (Constants.isTestnet ? 12908200 : 14271000)
                || Lrd.getBlockchain().getHeight() >= Constants.LQT_BLOCK);
    }
    ...
    public void validate() throws LrdException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeLQT != 0) : (deadline < 1 || feeLQT <= 0)
                || feeLQT > Constants.MAX_BALANCE_LQT
                || amountLQT < 0
                || amountLQT > Constants.MAX_BALANCE_LQT
                || type == null) {
            throw new LrdException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeLQT + ", amount: " + amountLQT);
        }

        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new LrdException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new LrdException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountLQT() != 0) {
                throw new LrdException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new LrdException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new LrdException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new LrdException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        long minimumFeeLQT = getMinimumFeeLQT(Lrd.getBlockchain().getHeight());
        if (feeLQT < minimumFeeLQT) {
            throw new LrdException.NotCurrentlyValidException(String.format("Transaction fee %f LRD less than minimum fee %f LRD at height %d",
                    ((double)feeLQT)/Constants.ONE_LRD, ((double)minimumFeeLQT)/Constants.ONE_LRD, Lrd.getBlockchain().getHeight()));
        }
    }
    ...
    void apply() {
        ...
        if (phasing != null && type.isPhasable()) {
            senderAccount.addToBalanceLQT(-feeLQT);
        }
        for (Appendix.AbstractAppendix appendage : appendages) {
    ...
    private long getMinimumFeeLQT(int blockchainHeight) {
        long totalFee = 0;
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.loadPrunable(this);
