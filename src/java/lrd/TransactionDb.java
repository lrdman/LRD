...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws LrdException.NotValidException {
        try {
            ...
            long amountLQT = rs.getLong("amount");
            long feeLQT = rs.getLong("fee");
            ...
            TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, null,
                    amountLQT, feeLQT, deadline, transactionType.parseAttachment(buffer, version))
                    .timestamp(timestamp)
        ...
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    pstmt.setLong(++i, transaction.getAmountLQT());
                    pstmt.setLong(++i, transaction.getFeeLQT());
