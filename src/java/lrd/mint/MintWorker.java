...
public class MintWorker {
    ...
    private void mint() {
        String currencyCode = Convert.emptyToNull(Lrd.getStringProperty("lrd.mint.currencyCode"));
        if (currencyCode == null) {
            throw new IllegalArgumentException("lrd.mint.currencyCode not specified");
        }
        String secretPhrase = Convert.emptyToNull(Lrd.getStringProperty("lrd.mint.secretPhrase", null, true));
        if (secretPhrase == null) {
            throw new IllegalArgumentException("lrd.mint.secretPhrase not specified");
        }
        boolean isSubmitted = Lrd.getBooleanProperty("lrd.mint.isSubmitted");
        boolean isStopOnError = Lrd.getBooleanProperty("lrd.mint.stopOnError");
        ...
        String unitsStr = Lrd.getStringProperty("lrd.mint.unitsPerMint");
        double wholeUnits = 1;
        ...
        long initialNonce = Lrd.getIntProperty("lrd.mint.initialNonce");
        if (initialNonce == 0) {
            initialNonce = new Random().nextLong();
        }
        int threadPoolSize = Lrd.getIntProperty("lrd.mint.threadPoolSize");
        if (threadPoolSize == 0) {
    ...
    private JSONObject mintImpl(String secretPhrase, long accountId, long units, long currencyId, byte algorithm,
        ...
        Logger.logInfoMessage("solution nonce %d unitsLQT %d counter %d computed hashes %d time [sec] %.2f hash rate [KH/Sec] %d actual time vs. expected %.2f is submitted %b",
        ...
        if (isSubmitted) {
            ...
            response.put("message", "lrd.mint.isSubmitted=false therefore currency mint transaction is not submitted");
        }
        return response;
    }
    ...
    private JSONObject currencyMint(String secretPhrase, long currencyId, long nonce, long units, long counter) {
        ...
        Transaction.Builder builder = Lrd.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, Constants.ONE_LRD,
        ...
        } catch (LrdException.NotValidException e) {
    ...
    private JSONObject getJsonResponse(Map<String, String> params) {
        ...
        String host = Convert.emptyToNull(Lrd.getStringProperty("lrd.mint.serverAddress"));
        if (host == null) {
            try {
        ...
        boolean useHttps = Lrd.getBooleanProperty("lrd.mint.useHttps");
        if (useHttps) {
            protocol = "https";
            HttpsURLConnection.setDefaultSSLSocketFactory(sslSocketFactory);
            HttpsURLConnection.setDefaultHostnameVerifier(hostNameVerifier);
        }
        int port = Constants.isTestnet ? API.TESTNET_API_PORT : Lrd.getIntProperty("lrd.apiServerPort");
        String urlParams = getUrlParams(params);
        URL url;
        try {
            url = new URL(protocol, host, port, "/lrd?" + urlParams);
