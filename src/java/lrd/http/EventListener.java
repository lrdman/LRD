 ...
 * has not been received within lrd.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by lrd.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Lrd.getIntProperty("lrd.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Lrd.getIntProperty("lrd.apiEventTimeout"), 15);
    ...
    void addEvents(List<Peers.Event> peerEvents, List<BlockchainProcessor.Event> blockEvents,
                                        List<TransactionProcessor.Event> txEvents) {
        BlockchainProcessor blockProcessor = Lrd.getBlockchainProcessor();
        TransactionProcessor txProcessor = Lrd.getTransactionProcessor();
        lock.lock();
        try {
            if (deactivated)
                return;
