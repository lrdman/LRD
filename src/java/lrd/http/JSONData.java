...
import lrd.Appendix;
import lrd.Asset;
import lrd.AssetTransfer;
import lrd.Attachment;
import lrd.Block;
import lrd.Constants;
import lrd.Currency;
import lrd.CurrencyExchangeOffer;
import lrd.CurrencyFounder;
import lrd.CurrencyTransfer;
import lrd.CurrencyType;
import lrd.DigitalGoodsStore;
import lrd.Exchange;
import lrd.ExchangeRequest;
import lrd.Generator;
import lrd.MonetarySystem;
import lrd.Lrd;
import lrd.Order;
import lrd.PhasingPoll;
import lrd.PhasingVote;
import lrd.Poll;
...
final class JSONData {
    ...
    static JSONObject alias(Alias alias) {
        ...
        if (offer != null) {
            json.put("priceLQT", String.valueOf(offer.getPriceLQT()));
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        JSONObject json = new JSONObject();
        if (account == null) {
            json.put("balanceLQT", "0");
            json.put("unconfirmedBalanceLQT", "0");
            json.put("forgedBalanceLQT", "0");
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceLRD", "0");
                json.put("guaranteedBalanceLQT", "0");
            }
        } else {
            json.put("balanceLQT", String.valueOf(account.getBalanceLQT()));
            json.put("unconfirmedBalanceLQT", String.valueOf(account.getUnconfirmedBalanceLQT()));
            json.put("forgedBalanceLQT", String.valueOf(account.getForgedBalanceLQT()));
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceLRD", account.getEffectiveBalanceLRD());
                json.put("guaranteedBalanceLQT", String.valueOf(account.getGuaranteedBalanceLQT()));
            }
        }
        return json;
    }
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceLRD", String.valueOf(account.getGuaranteedBalanceLQT() / Constants.ONE_LRD));
            }
        }
    ...
    static JSONObject currency(Currency currency, boolean includeCounts) {
        ...
        json.put("minReservePerUnitLQT", String.valueOf(currency.getMinReservePerUnitLQT()));
        json.put("currentReservePerUnitLQT", String.valueOf(currency.getCurrentReservePerUnitLQT()));
    ...
    static JSONObject currencyFounder(CurrencyFounder founder) {
        ...
        json.put("amountPerUnitLQT", String.valueOf(founder.getAmountPerUnitLQT()));
        return json;
    }
    ...
    private static JSONObject order(Order order) {
        ...
        json.put("priceLQT", String.valueOf(order.getPriceLQT()));
    ...
    private static JSONObject expectedOrder(Transaction transaction) {
        ...
        json.put("priceLQT", String.valueOf(attachment.getPriceLQT()));
    ...
    static JSONObject offer(CurrencyExchangeOffer offer) {
        ...
        json.put("rateLQT", String.valueOf(offer.getRateLQT()));
    ...
    static JSONObject expectedBuyOffer(Transaction transaction) {
        ...
        json.put("rateLQT", String.valueOf(attachment.getBuyRateLQT()));
    ...
    static JSONObject expectedSellOffer(Transaction transaction) {
        JSONObject json = expectedOffer(transaction);
        Attachment.MonetarySystemPublishExchangeOffer attachment = (Attachment.MonetarySystemPublishExchangeOffer)transaction.getAttachment();
        json.put("rateLQT", String.valueOf(attachment.getSellRateLQT()));
    ...
    static JSONObject block(Block block, boolean includeTransactions) {
        ...
        json.put("totalAmountLQT", String.valueOf(block.getTotalAmountLQT()));
        json.put("totalFeeLQT", String.valueOf(block.getTotalFeeLQT()));
    ...
    static JSONObject goods(DigitalGoodsStore.Goods goods, boolean includeCounts) {
        ...
        json.put("priceLQT", String.valueOf(goods.getPriceLQT()));
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Lrd.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject purchase(DigitalGoodsStore.Purchase purchase) {
        ...
        json.put("priceLQT", String.valueOf(purchase.getPriceLQT()));
        ...
        if (purchase.getDiscountLQT() > 0) {
            json.put("discountLQT", String.valueOf(purchase.getDiscountLQT()));
        }
        if (purchase.getRefundLQT() > 0) {
            json.put("refundLQT", String.valueOf(purchase.getRefundLQT()));
        }
        return json;
    }
    ...
    static JSONObject trade(Trade trade, boolean includeAssetInfo) {
        ...
        json.put("priceLQT", String.valueOf(trade.getPriceLQT()));
    ...
    static JSONObject exchange(Exchange exchange, boolean includeCurrencyInfo) {
        ...
        json.put("rateLQT", String.valueOf(exchange.getRate()));
    ...
    static JSONObject exchangeRequest(ExchangeRequest exchangeRequest, boolean includeCurrencyInfo) {
        ...
        json.put("rateLQT", String.valueOf(exchangeRequest.getRate()));
    ...
    static JSONObject expectedExchangeRequest(Transaction transaction, boolean includeCurrencyInfo) {
        ...
        json.put("units", String.valueOf(attachment.getUnits()));
        json.put("rateLQT", String.valueOf(attachment.getRateLQT()));
        if (includeCurrencyInfo) {
            putCurrencyInfo(json, attachment.getCurrencyId());
        }
        putExpectedTransaction(json, transaction);
    ...
    static JSONObject unconfirmedTransaction(Transaction transaction) {
        ...
        json.put("amountLQT", String.valueOf(transaction.getAmountLQT()));
        json.put("feeLQT", String.valueOf(transaction.getFeeLQT()));
    ...
    static JSONObject transaction(Transaction transaction) {
        ...
        json.put("confirmations", Lrd.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Lrd.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
