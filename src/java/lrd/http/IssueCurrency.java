...
/**
 * Issue a currency on the LRD blockchain
 * <p>
 * A currency is the basic block of the LRD Monetary System it can be exchanged with LRD, transferred between accounts,
 ...
 * For {@link lrd.CurrencyType#RESERVABLE} currency
 * <li>minReservePerUnitLQT - the minimum LRD value per unit to allow the currency to become active
 * For {@link lrd.CurrencyType#RESERVABLE} currency
 ...
 * For {@link lrd.CurrencyType#MINTABLE} currency
 * <li>maxDifficulty - for mint-able currency, the exponent of the final difficulty.
 * For {@link lrd.CurrencyType#MINTABLE} currency
 * <li>algorithm - the hashing {@link lrd.crypto.HashFunction algorithm} used for minting.
 * For {@link lrd.CurrencyType#MINTABLE} currency
 * </ul>
 * <p>
 * Constraints
 * <ul>
 * <li>A given currency can not be neither {@link lrd.CurrencyType#EXCHANGEABLE} nor {@link lrd.CurrencyType#CLAIMABLE}.<br>
 * <li>A {@link lrd.CurrencyType#RESERVABLE} currency becomes active once the blockchain height reaches the currency issuance height.<br>
 * At this time, if the minReservePerUnitLQT has not been reached the currency issuance is cancelled and
 ...
 * When a {@link lrd.CurrencyType#RESERVABLE} becomes active, in case it is {@link lrd.CurrencyType#CLAIMABLE} the LRD used for
 * reserving the currency are locked until they are claimed back.
 * When a {@link lrd.CurrencyType#RESERVABLE} becomes active, in case it is non {@link lrd.CurrencyType#CLAIMABLE} the LRD used for
 * reserving the currency are sent to the issuer account as crowd funding.
 * <li>When issuing a {@link lrd.CurrencyType#MINTABLE} currency, the number of units per {@link lrd.http.CurrencyMint} cannot exceed 0.01% of the
 ...
 * difficulty increases linearly with the number units minted per {@link lrd.http.CurrencyMint}<br>
 * </ul>
 *
 * @see lrd.CurrencyType
 * @see lrd.crypto.HashFunction
 */
public final class IssueCurrency extends CreateTransaction {
    ...
    private IssueCurrency() {
        super(new APITag[] {APITag.MS, APITag.CREATE_TRANSACTION},
                "name", "code", "description", "type", "initialSupply", "reserveSupply", "maxSupply", "issuanceHeight", "minReservePerUnitLQT",
    ...
    JSONStreamAware processRequest(HttpServletRequest req) throws LrdException {
        ...
        long minReservePerUnit = ParameterParser.getLong(req, "minReservePerUnitLQT", 1, Constants.MAX_BALANCE_LQT, false);
