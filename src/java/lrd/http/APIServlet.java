...
import static lrd.http.JSONResponses.ERROR_INCORRECT_REQUEST;
import static lrd.http.JSONResponses.ERROR_NOT_ALLOWED;
import static lrd.http.JSONResponses.POST_REQUIRED;
import static lrd.http.JSONResponses.REQUIRED_BLOCK_NOT_FOUND;
import static lrd.http.JSONResponses.REQUIRED_LAST_BLOCK_NOT_FOUND;
...
public final class APIServlet extends HttpServlet {
    ...
    abstract static class APIRequestHandler {
        ...
        abstract JSONStreamAware processRequest(HttpServletRequest request) throws LrdException;

        JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws LrdException {
    ...
    private static final boolean enforcePost = Lrd.getBooleanProperty("lrd.apiServerEnforcePOST");
            ...
            try {
                ...
                try {
                    try {
                        ...
                        if (requireBlockId != 0 && !Lrd.getBlockchain().hasBlock(requireBlockId)) {
                            response = REQUIRED_BLOCK_NOT_FOUND;
                            return;
                        }
                        if (requireLastBlockId != 0 && requireLastBlockId != Lrd.getBlockchain().getLastBlock().getId()) {
                        ...
                        if (requireLastBlockId == 0 && requireBlockId != 0 && response instanceof JSONObject) {
                            ((JSONObject) response).put("lastBlock", Lrd.getBlockchain().getLastBlock().getStringId());
                        }
                    } finally {
                        if (apiRequestHandler.startDbTransaction()) {
