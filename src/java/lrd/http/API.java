...
import static lrd.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static lrd.http.JSONResponses.NO_PASSWORD_IN_CONFIG;

public final class API {

    public static final int TESTNET_API_PORT = 7777;
    public static final int TESTNET_API_SSLPORT = 7778;

    private static final Set<String> allowedBotHosts;
    private static final List<NetworkAddress> allowedBotNets;
    static final String adminPassword = Lrd.getStringProperty("lrd.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Lrd.getIntProperty("lrd.maxAPIRecords");
    static final boolean enableAPIUPnP = Lrd.getBooleanProperty("lrd.enableAPIUPnP");

    private static final Server apiServer;
    private static URI browserUri;

    static {
        List<String> allowedBotHostsList = Lrd.getStringListProperty("lrd.allowedBotHosts");
        ...
        boolean enableAPIServer = Lrd.getBooleanProperty("lrd.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Lrd.getIntProperty("lrd.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Lrd.getIntProperty("lrd.apiServerSSLPort");
            final String host = Lrd.getStringProperty("lrd.apiServerHost");
            disableAdminPassword = Lrd.getBooleanProperty("lrd.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Lrd.getBooleanProperty("lrd.apiSSL");
            //
            ...
            if (!enableSSL || port != sslPort) {
                ...
                connector.setIdleTimeout(Lrd.getIntProperty("lrd.apiServerIdleTimeout"));
            ...
            if (enableSSL) {
                ...
                sslContextFactory.setKeyStorePath(Lrd.getStringProperty("lrd.keyStorePath"));
                sslContextFactory.setKeyStorePassword(Lrd.getStringProperty("lrd.keyStorePassword", null, true));
            ...
            String apiResourceBase = Lrd.getStringProperty("lrd.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Lrd.getStringProperty("lrd.apiWelcomeFile")});
            }

            String javadocResourceBase = Lrd.getStringProperty("lrd.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/lrd");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Lrd.getIntProperty("lrd.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));
            if (Lrd.getBooleanProperty("lrd.enableAPIServerGZIPFilter")) {
                FilterHolder gzipFilterHolder = apiHandler.addFilter(GzipFilter.class, "/lrd", null);
            ...
            if (Lrd.getBooleanProperty("lrd.apiServerCORS")) {
