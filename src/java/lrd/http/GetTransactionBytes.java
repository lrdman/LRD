...
import static lrd.http.JSONResponses.INCORRECT_TRANSACTION;
import static lrd.http.JSONResponses.MISSING_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = Lrd.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = Lrd.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", Lrd.getBlockchain().getHeight() - transaction.getHeight());
        }
