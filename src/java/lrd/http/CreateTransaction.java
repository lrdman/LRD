...
import static lrd.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static lrd.http.JSONResponses.INCORRECT_DEADLINE;
import static lrd.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static lrd.http.JSONResponses.INCORRECT_WHITELIST;
import static lrd.http.JSONResponses.MISSING_DEADLINE;
import static lrd.http.JSONResponses.MISSING_SECRET_PHRASE;
import static lrd.http.JSONResponses.NOT_ENOUGH_FUNDS;

abstract class CreateTransaction extends APIServlet.APIRequestHandler {

    private static final String[] commonParameters = new String[]{"secretPhrase", "publicKey", "feeLQT",
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId, long amountLQT)
            throws LrdException {
        return createTransaction(req, senderAccount, recipientId, amountLQT, Attachment.ORDINARY_PAYMENT);
    }
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        ...
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Lrd.getBlockchain().getHeight() + 1,
                Lrd.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountLQT, Attachment attachment)
            throws LrdException {
        ...
        try {
            Transaction.Builder builder = Lrd.newTransactionBuilder(publicKey, amountLQT, feeLQT,
            ...
            try {
                if (Math.addExact(amountLQT, transaction.getFeeLQT()) > senderAccount.getUnconfirmedBalanceLQT()) {
            ...
            } catch (LrdException.NotYetEncryptedException ignore) {}
        ...
        } catch (LrdException.NotYetEnabledException e) {
