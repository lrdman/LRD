...
public final class Currency {
    ...
    private static final class CurrencySupply {
        ...
        private long currentReservePerUnitLQT;
        ...
        private CurrencySupply(ResultSet rs) throws SQLException {
            ...
            this.currentReservePerUnitLQT = rs.getLong("current_reserve_per_unit_lqt");
        }

        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency_supply (id, current_supply, "
                    + "current_reserve_per_unit_lqt, height, latest) "
                    + "KEY (id, height) VALUES (?, ?, ?, ?, TRUE)")) {
                int i = 0;
                pstmt.setLong(++i, this.currencyId);
                pstmt.setLong(++i, this.currentSupply);
                pstmt.setLong(++i, this.currentReservePerUnitLQT);
    ...
    static {
        Lrd.getBlockchainProcessor().addListener(new CrowdFundingListener(), BlockchainProcessor.Event.AFTER_BLOCK_APPLY);
    }
    ...
    private Currency(Transaction transaction, Attachment.MonetarySystemCurrencyIssuance attachment) {
        ...
        this.minReservePerUnitLQT = attachment.getMinReservePerUnitLQT();
    ...
    private Currency(ResultSet rs) throws SQLException {
        ...
        this.minReservePerUnitLQT = rs.getLong("min_reserve_per_unit_lqt");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency (id, account_id, name, code, "
                + "description, type, initial_supply, reserve_supply, max_supply, creation_height, issuance_height, min_reserve_per_unit_lqt, "
            ...
            pstmt.setLong(++i, this.minReservePerUnitLQT);
    ...
    public long getCurrentReservePerUnitLQT() {
        if (!is(CurrencyType.RESERVABLE) || getSupplyData() == null) {
            return 0;
        }
        return currencySupply.currentReservePerUnitLQT;
    }
    ...
    static void increaseReserve(Account account, long currencyId, long amountPerUnitLQT) {
        Currency currency = Currency.getCurrency(currencyId);
        account.addToBalanceLQT(-Math.multiplyExact(currency.getReserveSupply(), amountPerUnitLQT));
        CurrencySupply currencySupply = currency.getSupplyData();
        currencySupply.currentReservePerUnitLQT += amountPerUnitLQT;
        currencySupplyTable.insert(currencySupply);
        CurrencyFounder.addOrUpdateFounder(currencyId, account.getId(), amountPerUnitLQT);
    }
    ...
    static void claimReserve(Account account, long currencyId, long units) {
        ...
        account.addToBalanceAndUnconfirmedBalanceLQT(Math.multiplyExact(units, currency.getCurrentReservePerUnitLQT()));
    }
            ...
            if (!isActive()) {
                try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currencyId, 0, Integer.MAX_VALUE)) {
                    for (CurrencyFounder founder : founders) {
                        Account.getAccount(founder.getAccountId()).addToBalanceAndUnconfirmedBalanceLQT(Math.multiplyExact(reserveSupply, founder.getAmountPerUnitLQT()));
                    }
                }
            }
    ...
    private static final class CrowdFundingListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            ...
            try (DbIterator<Currency> issuedCurrencies = currencyTable.getManyBy(new DbClause.IntClause("issuance_height", block.getHeight()), 0, -1)) {
                for (Currency currency : issuedCurrencies) {
                    if (currency.getCurrentReservePerUnitLQT() < currency.getMinReservePerUnitLQT()) {
        ...
        private void undoCrowdFunding(Currency currency) {
            try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
                for (CurrencyFounder founder : founders) {
                    Account.getAccount(founder.getAccountId()).addToBalanceAndUnconfirmedBalanceLQT(Math.multiplyExact(currency.getReserveSupply(), founder.getAmountPerUnitLQT()));
                }
            }
        ...
        private void distributeCurrency(Currency currency) {
            ...
            if (!currency.is(CurrencyType.CLAIMABLE)) {
                issuerAccount.addToBalanceAndUnconfirmedBalanceLQT(Math.multiplyExact(totalAmountPerUnit, currency.getReserveSupply()));
            }
