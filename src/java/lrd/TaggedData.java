...
import lrd.db.VersionedPersistentDbTable;
import lrd.db.VersionedPrunableDbTable;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = Lrd.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        ...
        this.blockTimestamp = Lrd.getBlockchain().getLastBlockTimestamp();
    ...
    static void add(Transaction transaction, Attachment.TaggedDataUpload attachment) {
        if (Lrd.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (Lrd.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = Lrd.getBlockchain().getLastBlockTimestamp();
                taggedDataTable.insert(taggedData);
            }
        }
    }
