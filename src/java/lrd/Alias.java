...
public final class Alias {
    ...
    public static class Offer {
        ...
        private Offer(long aliasId, long priceLQT, long buyerId) {
    ...
    static void addOrUpdateAlias(Transaction transaction, Attachment.MessagingAliasAssignment attachment) {
        ...
        if (alias == null) {
            ...
            alias.timestamp = Lrd.getBlockchain().getLastBlockTimestamp();
        }
    ...
    static void sellAlias(Transaction transaction, Attachment.MessagingAliasSell attachment) {
        ...
        final long priceLQT = attachment.getPriceLQT();
        final long buyerId = transaction.getRecipientId();
        if (priceLQT > 0) {
            Alias alias = getAlias(aliasName);
            Offer offer = getOffer(alias);
            if (offer == null) {
                offerTable.insert(new Offer(alias.id, priceLQT, buyerId));
            } else {
                offer.priceLQT = priceLQT;
