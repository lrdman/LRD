...
/**
 * Handle logging for the Lrd node server
 */
public final class Logger {
     ...
     * The lrd/conf/logging-default.properties and lrd/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "lrd.util.LrdLogManager");
        if (!(LogManager.getLogManager() instanceof LrdLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("lrd.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Lrd.loadProperties(loggingProperties, "logging-default.properties", true);
                Lrd.loadProperties(loggingProperties, "logging.properties", false);
                Lrd.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(lrd.Lrd.class);
        enableStackTraces = Lrd.getBooleanProperty("lrd.enableStackTraces");
        enableLogTraceback = Lrd.getBooleanProperty("lrd.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof LrdLogManager) {
            ((LrdLogManager) LogManager.getLogManager()).lrdShutdown();
        }
    }
