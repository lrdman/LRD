...
/**
 * Java LogManager extension for use with Lrd
 */
public class LrdLogManager extends LogManager {

    /** Logging reconfiguration in progress */
    private volatile boolean loggingReconfiguration = false;

    /**
     * Create the Lrd log manager
     *
     ...
     * done terminating the Lrd processes.
     */
    public LrdLogManager() {
        super();
    }
     ...
     * This allows us to continue to use logging facilities during Lrd shutdown.
     */
    @Override
    public void reset() {
    ...
    /**
     * Lrd shutdown is now complete, so call LogManager.reset() to terminate
     * the log handlers.
     */
    void lrdShutdown() {
        super.reset();
