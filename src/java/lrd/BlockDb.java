...
final class BlockDb {
    ...
    static BlockImpl loadBlock(Connection con, ResultSet rs, boolean loadTransactions) {
        try {
            ...
            long totalAmountLQT = rs.getLong("total_amount");
            long totalFeeLQT = rs.getLong("total_fee");
            ...
            return new BlockImpl(version, timestamp, previousBlockId, totalAmountLQT, totalFeeLQT, payloadLength, payloadHash,
    ...
    static void saveBlock(Connection con, BlockImpl block) {
        try {
            try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO block (id, version, timestamp, previous_block_id, "
                ...
                pstmt.setLong(++i, block.getTotalAmountLQT());
                pstmt.setLong(++i, block.getTotalFeeLQT());
