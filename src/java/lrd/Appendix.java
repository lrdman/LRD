...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Constants.INCLUDE_EXPIRED_PRUNABLE ||
                    Lrd.getEpochTime() - transaction.getTimestamp() <
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        abstract void validate(Transaction transaction) throws LrdException.ValidationException;

        void validateAtFinish(Transaction transaction) throws LrdException.ValidationException {
    ...
    class Message extends AbstractAppendix {
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            ...
            if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new LrdException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
            if (isText && !Arrays.equals(message, Convert.toBytes(Convert.toString(message)))) {
                throw new LrdException.NotValidException("Message is not UTF-8 text");
            }
        }
        ...
        @Override
        void validate(Transaction transaction) throws LrdException.ValidationException {
            if (message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new LrdException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_LRD/10) {
            @Override
        ...
        PrunablePlainMessage(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            super(buffer, transactionVersion);
        ...
        void validate(Transaction transaction) throws LrdException.ValidationException {
            if (transaction.getMessage() != null) {
                throw new LrdException.NotValidException("Cannot have both message and prunable message attachments");
            }
            byte[] msg = getMessage();
            if (msg != null && msg.length > Constants.MAX_PRUNABLE_MESSAGE_LENGTH) {
                throw new LrdException.NotValidException("Invalid prunable message length: " + msg.length);
            }
            if (msg == null && Lrd.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new LrdException.NotCurrentlyValidException("Message has been pruned prematurely");
            }
        }
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            super(buffer, transactionVersion);
        ...
        void validate(Transaction transaction) throws LrdException.ValidationException {
            if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new LrdException.NotValidException("Max encrypted message length exceeded");
            }
            if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                ...
                throw new LrdException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
            }
            if ((getVersion() != 2 && !isCompressed) || (getVersion() == 2 && isCompressed)) {
                throw new LrdException.NotValidException("Version mismatch - version " + getVersion() + ", isCompressed " + isCompressed);
            }
        }
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_LRD/10) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws LrdException.ValidationException {
            if (transaction.getEncryptedMessage() != null) {
                throw new LrdException.NotValidException("Cannot have both encrypted and prunable encrypted message attachments");
            }
            if (transaction.getPrunablePlainMessage() != null) {
                throw new LrdException.NotValidException("Cannot have both plain and encrypted prunable message attachments");
            }
            EncryptedData ed = getEncryptedData();
            if (ed == null && Lrd.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new LrdException.NotCurrentlyValidException("Encrypted message has been pruned prematurely");
            }
            if (ed != null) {
                if (ed.getData().length > Constants.MAX_PRUNABLE_ENCRYPTED_MESSAGE_LENGTH) {
                    throw new LrdException.NotValidException(String.format("Message length %d exceeds max prunable encrypted message length %d",
                ...
                if ((ed.getNonce().length != 32 && ed.getData().length > 0)
                    ...
                    throw new LrdException.NotValidException("Invalid nonce length " + ed.getNonce().length);
                }
            }
            if (transaction.getRecipientId() == 0) {
                throw new LrdException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
        }
    ...
    final class UnencryptedPrunableEncryptedMessage extends PrunableEncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new LrdException.NotYetEncryptedException("Prunable encrypted message not yet encrypted");
            }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class UnencryptedEncryptedMessage extends EncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new LrdException.NotYetEncryptedException("Message not yet encrypted");
            }
            super.putMyBytes(buffer);
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws LrdException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new LrdException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (publicKey.length != 32) {
                throw new LrdException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new LrdException.NotValidException("Announced public key does not match recipient accountId");
            }
            ...
            if (recipientAccount != null && recipientAccount.getKeyHeight() > 0 && ! Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
                throw new LrdException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
    ...
    class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = new Fee.ConstantFee(20 * Constants.ONE_LRD);
        ...
        void validate(Transaction transaction) throws LrdException.ValidationException {
            ...
            int currentHeight = Lrd.getBlockchain().getHeight();

            if (whitelist.length > Constants.MAX_PHASING_WHITELIST_SIZE) {
                throw new LrdException.NotValidException("Whitelist is too big");
            }

            long previousAccountId = 0;
            for (long accountId : whitelist) {
                if (accountId == 0) {
                    throw new LrdException.NotValidException("Invalid accountId 0 in whitelist");
                }
                if (previousAccountId != 0 && accountId < previousAccountId) {
                    throw new LrdException.NotValidException("Whitelist not sorted " + Arrays.toString(whitelist));
                }
                if (accountId == previousAccountId) {
                    throw new LrdException.NotValidException("Duplicate accountId " + Long.toUnsignedString(accountId) + " in whitelist");
                }
                previousAccountId = accountId;
            }

            if (quorum <= 0 && voteWeighting.getVotingModel() != VoteWeighting.VotingModel.NONE) {
                throw new LrdException.NotValidException("quorum <= 0");
            }

            if (voteWeighting.getVotingModel() == VoteWeighting.VotingModel.NONE) {
                if (quorum != 0) {
                    throw new LrdException.NotValidException("Quorum must be 0 for no-voting phased transaction");
                }
                if (whitelist.length != 0) {
                    throw new LrdException.NotValidException("No whitelist needed for no-voting phased transaction");
                }
            }

            if (voteWeighting.getVotingModel() == VoteWeighting.VotingModel.ACCOUNT && whitelist.length > 0 && quorum > whitelist.length) {
                throw new LrdException.NotValidException("Quorum of " + quorum + " cannot be achieved in by-account voting with whitelist of length "
                        + whitelist.length);
            }
            ...
            if (voteWeighting.getVotingModel() == VoteWeighting.VotingModel.TRANSACTION) {
                if (linkedFullHashes.length == 0 || linkedFullHashes.length > Constants.MAX_PHASING_LINKED_TRANSACTIONS) {
                    throw new LrdException.NotValidException("Invalid number of linkedFullHashes " + linkedFullHashes.length);
                }
                for (byte[] hash : linkedFullHashes) {
                    if (Convert.emptyToNull(hash) == null || hash.length != 32) {
                        throw new LrdException.NotValidException("Invalid linkedFullHash " + Convert.toHexString(hash));
                    }
                    ...
                    if (linkedTransaction != null) {
                        if (transaction.getTimestamp() - linkedTransaction.getTimestamp() > Constants.MAX_REFERENCED_TRANSACTION_TIMESPAN) {
                            throw new LrdException.NotValidException("Linked transaction cannot be more than 60 days older than the phased transaction");
                        }
                        if (linkedTransaction.getPhasing() != null) {
                            throw new LrdException.NotCurrentlyValidException("Cannot link to an already existing phased transaction");
                        }
                    }
                }
                if (quorum > linkedFullHashes.length) {
                    throw new LrdException.NotValidException("Quorum of " + quorum + " cannot be achieved in by-transaction voting with "
                            + linkedFullHashes.length + " linked full hashes only");
                }
            } else {
                if (linkedFullHashes.length != 0) {
                    throw new LrdException.NotValidException("LinkedFullHashes can only be used with VotingModel.TRANSACTION");
                }
            }

            if (voteWeighting.getVotingModel() == VoteWeighting.VotingModel.HASH) {
                if (quorum != 1) {
                    throw new LrdException.NotValidException("Quorum must be 1 for by-hash voting");
                }
                if (hashedSecret.length == 0 || hashedSecret.length > Byte.MAX_VALUE) {
                    throw new LrdException.NotValidException("Invalid hashedSecret " + Convert.toHexString(hashedSecret));
                }
                if (PhasingPoll.getHashFunction(algorithm) == null) {
                    throw new LrdException.NotValidException("Invalid hashedSecretAlgorithm " + algorithm);
                }
            } else {
                if (hashedSecret.length != 0) {
                    throw new LrdException.NotValidException("HashedSecret can only be used with VotingModel.HASH");
                }
                if (algorithm != 0) {
                    throw new LrdException.NotValidException("HashedSecretAlgorithm can only be used with VotingModel.HASH");
                }
            }

            if (finishHeight <= currentHeight + (voteWeighting.acceptsVotes() ? 2 : 1)
                    || finishHeight >= currentHeight + Constants.MAX_PHASING_DURATION) {
                throw new LrdException.NotCurrentlyValidException("Invalid finish height " + finishHeight);
            }
        ...
        void reject(TransactionImpl transaction) {
            ...
            senderAccount.addToUnconfirmedBalanceLQT(transaction.getAmountLQT());
