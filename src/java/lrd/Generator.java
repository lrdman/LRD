...
public final class Generator implements Comparable<Generator> {
    ...
    static {
        byte[] publicKey = null;
        if (Lrd.getBooleanProperty("lrd.enableFakeForging")) {
            Account fakeForgingAccount = Account.getAccount(Convert.parseAccountId(Lrd.getStringProperty("lrd.fakeForgingAccount")));
            ...
            try {
                try {
                    ...
                    try {
                        ...
                        int generationLimit = Lrd.getEpochTime() - delayTime;
                        if (!logged) {
    ...
    static long getHitTime(Account account, Block block) {
        return getHitTime(BigInteger.valueOf(account.getEffectiveBalanceLRD(block.getHeight())), getHit(account.getPublicKey(), block), block);
    }
    ...
    private Generator(String secretPhrase) {
        ...
        if (Lrd.getBlockchain().getHeight() >= Constants.LAST_KNOWN_BLOCK) {
            setLastBlock(Lrd.getBlockchain().getLastBlock());
        }
        sortedForgers = null;
    }
    ...
    public long getDeadline() {
        return Math.max(hitTime - Lrd.getBlockchain().getLastBlock().getTimestamp(), 0);
    }
    ...
    private void setLastBlock(Block lastBlock) {
        ...
        effectiveBalance = BigInteger.valueOf(account == null || account.getEffectiveBalanceLRD() <= 0 ? 0 : account.getEffectiveBalanceLRD());
    ...
    boolean forge(Block lastBlock, int generationLimit) throws BlockchainProcessor.BlockNotAcceptedException {
        ...
        while (true) {
            try {
                ...
                if (Lrd.getEpochTime() - start > 10) { // give up after trying for 10 s
                    throw e;
                }
            }
        }
