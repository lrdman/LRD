...
public final class DigitalGoodsStore {
    ...
    static {
        Lrd.getBlockchainProcessor().addListener(block -> {
            ...
            for (Purchase purchase : expiredPurchases) {
                ...
                buyer.addToUnconfirmedBalanceLQT(Math.multiplyExact((long) purchase.getQuantity(), purchase.getPriceLQT()));
    ...
    public static final class Goods {
        ...
        private Goods(Transaction transaction, Attachment.DigitalGoodsListing attachment) {
            ...
            this.delisted = false;
            this.timestamp = Lrd.getBlockchain().getLastBlockTimestamp();
        }
        ...
        private void changePrice(long priceLQT) {
    ...
    public static final class Purchase {
        ...
        private static DbIterator<Purchase> getExpiredPendingPurchases(Block block) {
            ...
            final int previousTimestamp = Lrd.getBlockchain().getBlock(block.getPreviousBlockId()).getTimestamp();
        ...
        private long discountLQT;
        private long refundLQT;
        ...
        private Purchase(ResultSet rs) throws SQLException {
            ...
            this.discountLQT = rs.getLong("discount");
            this.refundLQT = rs.getLong("refund");
        }
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO purchase (id, buyer_id, goods_id, seller_id, "
                ...
                pstmt.setLong(++i, this.discountLQT);
                pstmt.setLong(++i, this.refundLQT);
                pstmt.setInt(++i, Lrd.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
        ...
        public long getDiscountLQT() {
            return discountLQT;
        }

        private void setDiscountLQT(long discountLQT) {
            this.discountLQT = discountLQT;
            purchaseTable.insert(this);
        }

        public long getRefundLQT() {
            return refundLQT;
        }

        private void setRefundLQT(long refundLQT) {
    ...
    static void changePrice(long goodsId, long priceLQT) {
        Goods goods = Goods.goodsTable.get(Goods.goodsDbKeyFactory.newKey(goodsId));
        if (! goods.isDelisted()) {
            goods.changePrice(priceLQT);
    ...
    static void purchase(Transaction transaction,  Attachment.DigitalGoodsPurchase attachment) {
        ...
        if (! goods.isDelisted()
                ...
                && attachment.getPriceLQT() == goods.getPriceLQT()) {
            ...
            buyer.addToUnconfirmedBalanceLQT(Math.multiplyExact((long) attachment.getQuantity(), attachment.getPriceLQT()));
    ...
    static void deliver(Transaction transaction, Attachment.DigitalGoodsDelivery attachment) {
        ...
        purchase.setPending(false);
        long totalWithoutDiscount = Math.multiplyExact((long) purchase.getQuantity(), purchase.getPriceLQT());
        Account buyer = Account.getAccount(purchase.getBuyerId());
        buyer.addToBalanceLQT(Math.subtractExact(attachment.getDiscountLQT(), totalWithoutDiscount));
        buyer.addToUnconfirmedBalanceLQT(attachment.getDiscountLQT());
        Account seller = Account.getAccount(transaction.getSenderId());
        seller.addToBalanceAndUnconfirmedBalanceLQT(Math.subtractExact(totalWithoutDiscount, attachment.getDiscountLQT()));
        purchase.setEncryptedGoods(attachment.getGoods(), attachment.goodsIsText());
        purchase.setDiscountLQT(attachment.getDiscountLQT());
        purchaseListeners.notify(purchase, Event.DELIVERY);
    }

    static void refund(long sellerId, long purchaseId, long refundLQT, Appendix.EncryptedMessage encryptedMessage) {
        ...
        seller.addToBalanceLQT(-refundLQT);
        Account buyer = Account.getAccount(purchase.getBuyerId());
        buyer.addToBalanceAndUnconfirmedBalanceLQT(refundLQT);
        if (encryptedMessage != null) {
            purchase.setRefundNote(encryptedMessage.getEncryptedData());
        }
        purchase.setRefundLQT(refundLQT);
