...
import lrd.db.DerivedDbTable;
import lrd.db.PersistentDbTable;
...
public final class Account {
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Lrd.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Lrd.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Lrd.getBlockchain().getHeight());
            }
        }
    ...
    private long balanceLQT;
    private long unconfirmedBalanceLQT;
    private long forgedBalanceLQT;
    ...
    private Account(ResultSet rs) throws SQLException {
        ...
        this.balanceLQT = rs.getLong("balance");
        this.unconfirmedBalanceLQT = rs.getLong("unconfirmed_balance");
        this.forgedBalanceLQT = rs.getLong("forged_balance");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account (id, creation_height, "
            ...
            pstmt.setLong(++i, this.balanceLQT);
            pstmt.setLong(++i, this.unconfirmedBalanceLQT);
            pstmt.setLong(++i, this.forgedBalanceLQT);
    ...
    public long getBalanceLQT() {
        return balanceLQT;
    }

    public long getUnconfirmedBalanceLQT() {
        return unconfirmedBalanceLQT;
    }

    public long getForgedBalanceLQT() {
        return forgedBalanceLQT;
    }

    public long getEffectiveBalanceLRD() {
		return getEffectiveBalanceLRD(Lrd.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceLRD(int height) {
		return getBalanceLQT() / Constants.ONE_LRD;
    }

    private long getLessorsGuaranteedBalanceLQT(int height) {
        ...
        for (int i = 0; i < lessors.size(); i++) {
            ...
            balances[i] = lessors.get(i).getBalanceLQT();
        }
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT account_id, SUM (additions) AS additions "
                     + "FROM account_guaranteed_balance, TABLE (id BIGINT=?) T WHERE account_id = T.id AND height > ? "
                             + (height < Lrd.getBlockchain().getHeight() ? " AND height <= ? " : "")
    ...
    public long getGuaranteedBalanceLQT() {
        return getGuaranteedBalanceLQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Lrd.getBlockchain().getHeight());
    }

    public long getGuaranteedBalanceLQT(final int numberOfConfirmations, final int currentHeight) {
        int height = currentHeight - numberOfConfirmations;
        if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Lrd.getBlockchainProcessor().getMinRollbackHeight()
                || height > Lrd.getBlockchain().getHeight()) {
        ...
        try (Connection con = Db.db.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                ...
                return Math.max(Math.subtractExact(balanceLQT, rs.getLong("additions")), 0);
            }
        } catch (SQLException e) {
            throw new RuntimeException(e.toString(), e);
        }
    ...
    void leaseEffectiveBalance(long lesseeId, short period) {
        ...
        if (lessee != null && lessee.getKeyHeight() > 0) {
            int height = Lrd.getBlockchain().getHeight();
    ...
    void apply(byte[] key) {
        ...
        if (this.keyHeight == 0) {
            this.keyHeight = Lrd.getBlockchain().getHeight();
    ...
    void addToBalanceLQT(long amountLQT) {
        if (amountLQT == 0) {
            return;
        }
        this.balanceLQT = Math.addExact(this.balanceLQT, amountLQT);
        addToGuaranteedBalanceLQT(amountLQT);
        checkBalance(this.id, this.balanceLQT, this.unconfirmedBalanceLQT);
        accountTable.insert(this);
        listeners.notify(this, Event.BALANCE);
    }

    void addToUnconfirmedBalanceLQT(long amountLQT) {
        if (amountLQT == 0) {
            return;
        }
        this.unconfirmedBalanceLQT = Math.addExact(this.unconfirmedBalanceLQT, amountLQT);
        checkBalance(this.id, this.balanceLQT, this.unconfirmedBalanceLQT);
        accountTable.insert(this);
        listeners.notify(this, Event.UNCONFIRMED_BALANCE);
    }

    void addToBalanceAndUnconfirmedBalanceLQT(long amountLQT) {
        if (amountLQT == 0) {
            return;
        }
        this.balanceLQT = Math.addExact(this.balanceLQT, amountLQT);
    ...
    void addToForgedBalanceLQT(long amountLQT) {
        if (amountLQT == 0) {
            return;
        }
        this.forgedBalanceLQT = Math.addExact(this.forgedBalanceLQT, amountLQT);
        accountTable.insert(this);
    }
    ...
    private void addToGuaranteedBalanceLQT(long amountLQT) {
        if (amountLQT <= 0) {
            return;
        }
        int blockchainHeight = Lrd.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    void payDividends(final long assetId, final int height, final long amountLQTPerQNT) {
        ...
        for (final AccountAsset accountAsset : accountAssets) {
            if (accountAsset.getAccountId() != this.id && accountAsset.getAccountId() != Genesis.CREATOR_ID && accountAsset.getQuantityQNT() != 0) {
                long dividend = Math.multiplyExact(accountAsset.getQuantityQNT(), amountLQTPerQNT);
                Account.getAccount(accountAsset.getAccountId()).addToBalanceAndUnconfirmedBalanceLQT(dividend);
                totalDividend += dividend;
            }
        }
        this.addToBalanceLQT(-totalDividend);
    }
