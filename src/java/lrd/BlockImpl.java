...
final class BlockImpl implements Block {
    ...
    private final long totalAmountLQT;
    private final long totalFeeLQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountLQT, long totalFeeLQT, int payloadLength, byte[] payloadHash,
        ...
        this(version, timestamp, previousBlockId, totalAmountLQT, totalFeeLQT, payloadLength, payloadHash,
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountLQT, long totalFeeLQT, int payloadLength, byte[] payloadHash,
        ...
        this.totalAmountLQT = totalAmountLQT;
        this.totalFeeLQT = totalFeeLQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountLQT, long totalFeeLQT, int payloadLength,
    ...
    @Override
    public long getTotalAmountLQT() {
        return totalAmountLQT;
    }

    @Override
    public long getTotalFeeLQT() {
        return totalFeeLQT;
    }
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("totalAmountLQT", totalAmountLQT);
        json.put("totalFeeLQT", totalFeeLQT);
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws LrdException.NotValidException {
        try {
            int version = ((Long) blockData.get("version")).intValue();
            int timestamp = ((Long) blockData.get("timestamp")).intValue();
            long previousBlock = Convert.parseUnsignedLong((String) blockData.get("previousBlock"));
            long totalAmountLQT = Convert.parseLong(blockData.get("totalAmountLQT"));
            long totalFeeLQT = Convert.parseLong(blockData.get("totalFeeLQT"));
            ...
            BlockImpl block = new BlockImpl(version, timestamp, previousBlock, totalAmountLQT, totalFeeLQT, payloadLength, payloadHash, generatorPublicKey,
                    generationSignature, blockSignature, previousBlockHash, blockTransactions);
            if (!block.checkSignature()) {
                throw new LrdException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountLQT / Constants.ONE_LRD));
                buffer.putInt((int) (totalFeeLQT / Constants.ONE_LRD));
            } else {
                buffer.putLong(totalAmountLQT);
                buffer.putLong(totalFeeLQT);
            }
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            Account account = Account.getAccount(getGeneratorId());
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceLRD();
    ...
    void apply() {
        ...
        generatorAccount.addToBalanceAndUnconfirmedBalanceLQT(totalFeeLQT);
        generatorAccount.addToForgedBalanceLQT(totalFeeLQT);
    }
