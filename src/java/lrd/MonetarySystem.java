...
public abstract class MonetarySystem extends TransactionType {
    ...
    public static final TransactionType CURRENCY_ISSUANCE = new MonetarySystem() {
        ...
        private final Fee FIVE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(40 * Constants.ONE_LRD);
        private final Fee FOUR_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(1000 * Constants.ONE_LRD);
        private final Fee THREE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(25000 * Constants.ONE_LRD);
        ...
        @Override
        Attachment.MonetarySystemCurrencyIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemCurrencyIssuance(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyIssuance parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getMaxSupply() > Constants.MAX_CURRENCY_TOTAL_SUPPLY
                    ...
                    || attachment.getMinReservePerUnitLQT() < 0
                    || attachment.getDecimals() < 0 || attachment.getDecimals() > 8
                    || attachment.getRuleset() != 0) {
                throw new LrdException.NotValidException("Invalid currency issuance: " + attachment.getJSONObject());
            }
            int t = 1;
            for (int i = 0; i < 32; i++) {
                if ((t & attachment.getType()) != 0 && CurrencyType.get(t) == null) {
                    throw new LrdException.NotValidException("Invalid currency type: " + attachment.getType());
                }
                t <<= 1;
            }
    ...
    public static final TransactionType RESERVE_INCREASE = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemReserveIncrease parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemReserveIncrease(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemReserveIncrease parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getAmountPerUnitLQT() <= 0) {
                throw new LrdException.NotValidException("Reserve increase LRD amount must be positive: " + attachment.getAmountPerUnitLQT());
            }
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            Attachment.MonetarySystemReserveIncrease attachment = (Attachment.MonetarySystemReserveIncrease) transaction.getAttachment();
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            if (senderAccount.getUnconfirmedBalanceLQT() >= Math.multiplyExact(currency.getReserveSupply(), attachment.getAmountPerUnitLQT())) {
                senderAccount.addToUnconfirmedBalanceLQT(-Math.multiplyExact(currency.getReserveSupply(), attachment.getAmountPerUnitLQT()));
                return true;
            }
            return false;
        }
        ...
        void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Lrd.getBlockchain().getTransaction(attachment.getCurrencyId());
            ...
            senderAccount.addToUnconfirmedBalanceLQT(Math.multiplyExact(reserveSupply, attachment.getAmountPerUnitLQT()));
        }

        @Override
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemReserveIncrease attachment = (Attachment.MonetarySystemReserveIncrease) transaction.getAttachment();
            Currency.increaseReserve(senderAccount, attachment.getCurrencyId(), attachment.getAmountPerUnitLQT());
        }
    ...
    public static final TransactionType RESERVE_CLAIM = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemReserveClaim parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemReserveClaim(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemReserveClaim parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new LrdException.NotValidException("Reserve claim number of units must be positive: " + attachment.getUnits());
            }
            CurrencyType.validate(Currency.getCurrency(attachment.getCurrencyId()), transaction);
        }
    ...
    public static final TransactionType CURRENCY_TRANSFER = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemCurrencyTransfer(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyTransfer parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new LrdException.NotValidException("Invalid currency transfer: " + attachment.getJSONObject());
            }
            if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                throw new LrdException.NotValidException("Currency transfer to genesis account not allowed");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new LrdException.NotCurrentlyValidException("Currency not currently active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType PUBLISH_EXCHANGE_OFFER = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemPublishExchangeOffer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemPublishExchangeOffer(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemPublishExchangeOffer parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getBuyRateLQT() <= 0
            		|| attachment.getSellRateLQT() <= 0
            		|| attachment.getBuyRateLQT() > attachment.getSellRateLQT()) {
                throw new LrdException.NotValidException(String.format("Invalid exchange offer, buy rate %d and sell rate %d has to be larger than 0, buy rate cannot be larger than sell rate",
                        attachment.getBuyRateLQT(), attachment.getSellRateLQT()));
            }
            if (attachment.getTotalBuyLimit() < 0
                ...
                throw new LrdException.NotValidException("Invalid exchange offer, units and height cannot be negative: " + attachment.getJSONObject());
            }
            if (attachment.getTotalBuyLimit() < attachment.getInitialBuySupply()
                ...
                throw new LrdException.NotValidException("Initial supplies must not exceed total limits");
            }
            if (attachment.getExpirationHeight() <= getFinishValidationHeight(transaction)) {
                throw new LrdException.NotCurrentlyValidException("Expiration height must be after transaction execution height");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            Attachment.MonetarySystemPublishExchangeOffer attachment = (Attachment.MonetarySystemPublishExchangeOffer) transaction.getAttachment();
            if (senderAccount.getUnconfirmedBalanceLQT() >= Math.multiplyExact(attachment.getInitialBuySupply(), attachment.getBuyRateLQT())
                    && senderAccount.getUnconfirmedCurrencyUnits(attachment.getCurrencyId()) >= attachment.getInitialSellSupply()) {
                senderAccount.addToUnconfirmedBalanceLQT(-Math.multiplyExact(attachment.getInitialBuySupply(), attachment.getBuyRateLQT()));
    ...
    abstract static class MonetarySystemExchange extends MonetarySystem {
        ...
        final void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (attachment.getRateLQT() <= 0 || attachment.getUnits() == 0) {
                throw new LrdException.NotValidException("Invalid exchange: " + attachment.getJSONObject());
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new LrdException.NotCurrentlyValidException("Currency not active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType EXCHANGE_BUY = new MonetarySystemExchange() {
        ...
        @Override
        Attachment.MonetarySystemExchangeBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemExchangeBuy(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemExchangeBuy parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            if (senderAccount.getUnconfirmedBalanceLQT() >= Math.multiplyExact(attachment.getUnits(), attachment.getRateLQT())) {
                senderAccount.addToUnconfirmedBalanceLQT(-Math.multiplyExact(attachment.getUnits(), attachment.getRateLQT()));
                return true;
            }
            return false;
        }
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            ...
            CurrencyExchangeOffer.exchangeLRDForCurrency(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateLQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType EXCHANGE_SELL = new MonetarySystemExchange() {
        ...
        @Override
        Attachment.MonetarySystemExchangeSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemExchangeSell(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemExchangeSell parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemExchangeSell attachment = (Attachment.MonetarySystemExchangeSell) transaction.getAttachment();
            ExchangeRequest.addExchangeRequest(transaction, attachment);
            CurrencyExchangeOffer.exchangeCurrencyForLRD(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateLQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType CURRENCY_MINTING = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyMinting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemCurrencyMinting(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyMinting parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            CurrencyType.validate(currency, transaction);
            if (attachment.getUnits() <= 0) {
                throw new LrdException.NotValidException("Invalid number of units: " + attachment.getUnits());
            }
            if (attachment.getUnits() > (currency.getMaxSupply() - currency.getReserveSupply()) / Constants.MAX_MINTING_RATIO) {
                throw new LrdException.NotValidException(String.format("Cannot mint more than 1/%d of the total units supply in a single request", Constants.MAX_MINTING_RATIO));
            }
            if (!currency.isActive()) {
                throw new LrdException.NotCurrentlyValidException("Currency not currently active " + attachment.getJSONObject());
            }
            ...
            if (attachment.getCounter() <= counter) {
                throw new LrdException.NotCurrentlyValidException(String.format("Counter %d has to be bigger than %d", attachment.getCounter(), counter));
            }
            if (!CurrencyMinting.meetsTarget(transaction.getSenderId(), currency, attachment)) {
                throw new LrdException.NotCurrentlyValidException(String.format("Hash doesn't meet target %s", attachment.getJSONObject()));
            }
        }
    ...
    public static final TransactionType CURRENCY_DELETION = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyDeletion parseAttachment(ByteBuffer buffer, byte transactionVersion) throws LrdException.NotValidException {
            return new Attachment.MonetarySystemCurrencyDeletion(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyDeletion parseAttachment(JSONObject attachmentData) throws LrdException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws LrdException.ValidationException {
            ...
            if (!currency.canBeDeletedBy(transaction.getSenderId())) {
                throw new LrdException.NotCurrentlyValidException("Currency " + Long.toUnsignedString(currency.getId()) + " cannot be deleted by account " +
