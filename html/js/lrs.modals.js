...
var LRS = (function(LRS, $, undefined) {
    ...
    modal.on("show.bs.modal", function() {
		...
		if ($visible_modal.length) {
			if ($visible_modal.hasClass("locked")) {
				...
				LRS.unlockForm($visible_modal, $btn, true);
			} else {
		...
		LRS.initAdvancedModalFormValues($(this));
	...
	modal.on("shown.bs.modal", function() {
		...
		LRS.showedFormWarning = false; //maybe not the best place... we assume forms are only in modals?
	});
	...
	modal.on("hidden.bs.modal", function() {
		...
		var $feeInput = $(this).find("input[name=feeLRD]");
		if ($feeInput.length) {
			var defaultFee = $feeInput.data("default");
			if (!defaultFee) {
				defaultFee = 1;
			}
			$(this).find(".advanced_fee").html(LRS.formatAmount(LRS.convertToLQT(defaultFee)) + " LRD");
		}
		LRS.showedFormWarning = false;
	});

	LRS.showModalError = function(errorMessage, $modal) {
	...
	$("input[name=feeLRD]").on("change", function() {
		var $modal = $(this).closest(".modal");
		var $feeInfo = $modal.find(".advanced_fee");
		if ($feeInfo.length) {
			$feeInfo.html(LRS.formatAmount(LRS.convertToLQT($(this).val())) + " LRD");
		}
	});
