...
var LRS = (function (LRS, $) {
	...
	LRS.generatePublicKey = function(secretPhrase) {
		if (!secretPhrase) {
		...
		return LRS.getPublicKey(converters.stringToHexString(secretPhrase));
	};

	LRS.getPublicKey = function(secretPhrase, isAccountNumber) {
		if (isAccountNumber) {
	...
	LRS.getPrivateKey = function(secretPhrase) {
	...
	LRS.getAccountId = function(secretPhrase) {
		return LRS.getAccountIdFromPublicKey(LRS.getPublicKey(converters.stringToHexString(secretPhrase)));
	};

	LRS.getAccountIdFromPublicKey = function(publicKey, RSFormat) {
	...
	LRS.encryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					...
					options.privateKey = converters.hexStringToByteArray(LRS.getPrivateKey(secretPhrase));
				}
				...
				if (!options.publicKey) {
					...
					try {
						options.publicKey = converters.hexStringToByteArray(LRS.getPublicKey(options.account, true));
					} catch (err) {
						var lrdAddress = new LrdAddress();

						if (!lrdAddress.set(options.account)) {
							throw {
	...
	LRS.decryptData = function(data, options, secretPhrase) {
		try {
			return LRS.decryptNote(message, options, secretPhrase);
		} catch (err) {
			if (err.errorCode && err.errorCode == 1) {
				return false;
			} else {
				if (options.title) {
					var translatedTitle = LRS.getTranslatedFieldName(options.title).toLowerCase();
					if (!translatedTitle) {
	...
	LRS.decryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					if (!secretPhrase) {
	...
	LRS.getSharedKeyWithAccount = function(account) {
		try {
			...
			var privateKey = converters.hexStringToByteArray(LRS.getPrivateKey(secretPhrase));

			var publicKey = converters.hexStringToByteArray(LRS.getPublicKey(account, true));
	...
	LRS.signBytes = function(message, secretPhrase) {
	...
	LRS.verifySignature = function(signature, message, publicKey, callback) {
	...
	LRS.setEncryptionPassword = function(password) {
		_password = password;
	};

	LRS.setDecryptionPassword = function(password) {
		_decryptionPassword = password;
	};

	LRS.addDecryptedTransaction = function(identifier, content) {
		if (!_decryptedTransactions[identifier]) {
			_decryptedTransactions[identifier] = content;
		}
	};

	LRS.tryToDecryptMessage = function(message) {
		...
		try {
			if (!message.attachment.encryptedMessage.data) {
				...
				var decoded = LRS.decryptNote(message.attachment.encryptedMessage.data, {
					"nonce": message.attachment.encryptedMessage.nonce,
					"account": (message.recipient == LRS.account ? message.sender : message.recipient)
				});
			}
	...
	LRS.tryToDecrypt = function(transaction, fields, account, options) {
		...
		if (!output) {
			$.each(fields, function(key, title) {
				...
				if (encrypted) {
					...
					try {
						data = LRS.decryptNote(encrypted, {
							"nonce": nonce,
							"account": account
						});
					} catch (err) {
						if (err.errorCode && err.errorCode == 1) {
							...
							if (title) {
								var translatedTitle = LRS.getTranslatedFieldName(title).toLowerCase();
								if (!translatedTitle) {
		...
		if (showDecryptionForm) {
			...
			LRS.removeDecryptionForm();
			$(outputEl).append(output).show();
		}
	};

	LRS.removeDecryptionForm = function($modal) {
	...
	LRS.decryptNoteFormSubmit = function() {
		...
		var accountId = LRS.getAccountId(password);
	...
	LRS.decryptAllMessages = function(messages, password) {
		if (!password) {
			throw {
				"message": $.t("error_passphrase_required"),
				"errorCode": 1
		...
		for (var i = 0; i < messages.length; i++) {
			...
			if (message.attachment.encryptedMessage && !_decryptedTransactions[message.transaction]) {
				try {
					var otherUser = (message.sender == LRS.account ? message.recipient : message.sender);
