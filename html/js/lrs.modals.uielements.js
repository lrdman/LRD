...
var LRS = (function(LRS, $, undefined) {
	...
	function _updateBlockHeightEstimates($bhmElem) {
		...
		if (blockHeight) {
			var blockDiff = blockHeight - LRS.lastBlockHeight;
			var diffSecs = blockDiff * LRS.averageBlockGenerationTime;
	...
	$("body").on("show.bs.modal", '.modal', function(e) {
		...
		$bhmElems.each(function(key, bhmElem) {
			...
			$(bhmElem).find(".bhm_ue_current_block_height").html(String(LRS.lastBlockHeight));
			$(bhmElem).find(".bhm_ue_use_current_block_height").data('CurrentBlockHeight', LRS.lastBlockHeight);
		});
		...
		LRS.loadAlgorithmList($algoSelect, ($(this).attr('id') != 'hash_modal'));
	});
