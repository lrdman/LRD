...
var LRS = (function(LRS, $, undefined) {
	LRS.showRawTransactionModal = function(transaction) {
    ...
    LRS.forms.broadcastTransaction = function(modal) {
        ...
        if (modal.attr('id') == "transaction_json_modal") {
            data = LRS.getFormData($("#broadcast_json_form"));
        } else {
            data = LRS.getFormData(modal.find("form:first"));
        }
	...
	LRS.initAdvancedModalFormValues = function($modal) {
		...
		//if (type != undefined && subType != undefined) {
			//if (LRS.transactionTypes[type]["subTypes"][subType]["serverConstants"]["isPhasingSafe"] == true) {
		...
		var $elems = LRS.initModalUIElement($modal, '.phasing_finish_height_group', 'block_height_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);

		context = {
			labelText: "Amount LRD",
			labelI18n: "amount_lrd",
			helpI18n: "approve_transaction_amount_help",
			inputName: "phasingQuorumLRD",
			addonText: "LRD",
			addonI18n: "lrd_unit"
		};
		$elems = LRS.initModalUIElement($modal, '.approve_transaction_amount_lrd', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = LRS.initModalUIElement($modal, '.approve_transaction_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = LRS.initModalUIElement($modal, '.approve_transaction_currency_units', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = LRS.initModalUIElement($modal, '.add_approval_whitelist_group', 'multi_accounts_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = LRS.initModalUIElement($modal, '.approve_min_balance_model_group', 'min_balance_model_modal_ui_element', context);
		...
		context['inputName'] = 'phasingMinBalanceLRD';
		context['addonText'] = 'LRD';
		context['addonI18n'] = 'lrd_unit';
		$elems = LRS.initModalUIElement($modal, '.approve_min_balance_lrd', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = LRS.initModalUIElement($modal, '.approve_min_balance_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = LRS.initModalUIElement($modal, '.approve_min_balance_currency_units', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = LRS.initModalUIElement($modal, '.approve_holding_asset', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems = LRS.initModalUIElement($modal, '.approve_holding_asset_optional', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = LRS.initModalUIElement($modal, '.approve_holding_currency', 'add_currency_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems = LRS.initModalUIElement($modal, '.approve_holding_currency_optional', 'add_currency_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		LRS.initModalUIElement($modal, '.hash_algorithm_model_group', 'hash_algorithm_model_modal_ui_element', context);
	...
	function _setApprovalFeeAddition($modal) {
		...
		var feeAddition = $modal.find('.approve_tab_list li.active a').data("feeLrdApprovalAddition");
        ...
        $modal.find("input[name='feeLRD_approval_addition']").val(feeAddition);
        $modal.find("span.feeLRD_approval_addition_info").html("+" + feeAddition);
	}
	...
	LRS.forms.broadcastTransactionComplete = function() {
		$("#parse_transaction_form").find(".error_message").hide();
        $("#transaction_json_modal").modal("hide");
	};

	LRS.forms.parseTransactionComplete = function(response) {
		$("#parse_transaction_form").find(".error_message").hide();
        var details = $.extend({}, response);
        if (response.attachment) {
            delete details.attachment;
        }
        $("#parse_transaction_output_table").find("tbody").empty().append(LRS.createInfoTable(details, true));
		$("#parse_transaction_output").show();
	};

	LRS.forms.parseTransactionError = function() {
		$("#parse_transaction_output_table").find("tbody").empty();
		$("#parse_transaction_output").hide();
	};

	LRS.forms.calculateFullHashComplete = function(response) {
		$("#calculate_full_hash_form").find(".error_message").hide();
		$("#calculate_full_hash_output_table").find("tbody").empty().append(LRS.createInfoTable(response, true));
		$("#calculate_full_hash_output").show();
	};

	LRS.forms.calculateFullHashError = function() {
    ...
    LRS.forms.signTransactionComplete = function(response) {
    ...
    LRS.forms.signTransaction = function() {
        var data = LRS.getFormData($("#sign_transaction_form"));
