...
var LRS = (function (LRS, $, undefined) {
    ...
    LRS.formatVolume = function (volume) {
    ...
    LRS.formatWeight = function (weight) {
        var digits = [],
            formattedWeight = "",
            i;
        do {
    ...
    LRS.formatOrderPricePerWholeQNT = function (price, decimals) {
        price = LRS.calculateOrderPricePerWholeQNT(price, decimals, true);

        return LRS.format(price);
    };

    LRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
        if (typeof price != "object") {
            price = new BigInteger(String(price));
        }

        return LRS.convertToLRD(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
    };

    LRS.calculatePricePerWholeQNT = function (price, decimals) {
        price = String(price);
    ...
    LRS.calculateOrderTotalLQT = function (quantityQNT, priceLQT) {
        if (typeof quantityQNT != "object") {
            quantityQNT = new BigInteger(String(quantityQNT));
        }

        if (typeof priceLQT != "object") {
            priceLQT = new BigInteger(String(priceLQT));
        }

        var orderTotal = quantityQNT.multiply(priceLQT);

        return orderTotal.toString();
    };

    LRS.calculateOrderTotal = function (quantityQNT, priceLQT) {
        if (typeof quantityQNT != "object") {
            quantityQNT = new BigInteger(String(quantityQNT));
        }
        ...
        return LRS.convertToLRD(quantityQNT.multiply(priceLQT));
    };

    LRS.calculatePercentage = function (a, b, rounding_mode) {
    ...
    LRS.convertToLRD = function (amount, returnAsObject) {
        var negative = "";
        var afterComma = "";
    ...
    LRS.amountToPrecision = function (amount, decimals) {
    ...
    LRS.convertToLQT = function (currency) {
    ...
    LRS.convertToQNTf = function (quantity, decimals, returnAsObject) {
    ...
    LRS.convertToQNT = function (quantity, decimals) {
        quantity = String(quantity);
    ...
    LRS.format = function (params, no_escaping) {
        var amount;
    ...
    LRS.formatQuantity = function (quantity, decimals, no_escaping) {
        return LRS.format(LRS.convertToQNTf(quantity, decimals, true), no_escaping);
    };

    LRS.formatAmount = function (amount, round, no_escaping) {
        ...
        if (typeof amount == "object") {
            var params = LRS.convertToLRD(amount, true);

            negative = params.negative;
            amount = params.amount;
            afterComma = params.afterComma;
        } else {
            //rounding only applies to non-lqt
            if (round) {
    ...
    LRS.fromEpochTime = function (epochTime) {
        if (LRS.constants.EPOCH_BEGINNING == 0) {
            throw "undefined epoch beginning";
        }
        return epochTime * 1000 + LRS.constants.EPOCH_BEGINNING - 500;
    };

    LRS.toEpochTime = function (currentTime) {
        ...
        return Math.floor((currentTime - LRS.constants.EPOCH_BEGINNING) / 1000);
    };

    LRS.formatTimestamp = function (timestamp, date_only, isAbsoluteTime) {
        var date;
        if (typeof timestamp == "object") {
            ...
            date = new Date(LRS.fromEpochTime(timestamp));
        }
        ...
        if (!isNaN(date) && typeof(date.getFullYear) == 'function') {
            ...
            if (!date_only) {
                ...
                if (LRS.settings["24_hour_format"] == "0") {
                    if (originalHours == 0) {
                        hours += 12;
    ...
    LRS.convertToHex16 = function (str) {
        var hex, i;
        var result = "";
    ...
    LRS.convertFromHex16 = function (hex) {
        var j;
    ...
    LRS.convertFromHex8 = function (hex) {
    ...
    LRS.convertToHex8 = function (str) {
        var hex = '';
    ...
    LRS.getFormData = function ($form, unmodified) {
    ...
    LRS.convertNumericToRSAccountFormat = function (account) {
        if (/^LRD\-/i.test(account)) {
    ...
    LRS.getAccountLink = function (object, accountKey, accountRef, title) {
        var accountRS;
        if (typeof object[accountKey + "RS"] != "undefined") {
            accountRS = object[accountKey + "RS"];
        } else if (typeof object[accountKey] != "undefined") {
            accountRS = LRS.convertNumericToRSAccountFormat(object[accountKey]);
        } else {
            return '/';
        }
        var accountTitle;
    ...
    LRS.getAccountTitle = function (object, acc) {
        ...
        if (formattedAcc == LRS.account || formattedAcc == LRS.accountRS) {
            return $.t("you");
        } else if (formattedAcc in LRS.contacts) {
            return LRS.contacts[formattedAcc].name.escapeHTML();
        } else {
            return String(formattedAcc).escapeHTML();
        }
    };

    LRS.getAccountFormatted = function (object, acc) {
        var type = typeof object;
    ...
    LRS.setupClipboardFunctionality = function () {
        ...
        if (LRS.inApp) {
            $el.on("click", function () {
                parent.postMessage({
                    "type": "copy",
                    "text": LRS.getClipboardText($(this).data("type"))
                }, "*");
            ...
            clipboard.on("dataRequested", function (client) {
                client.setText(LRS.getClipboardText($(this).data("type")));
            });
            ...
            if (!LRS.getCookie("clipboard_warning_shown")) {
                ...
                LRS.setCookie("clipboard_warning_shown", "1", 30);
            }
    ...
    LRS.getClipboardText = function (type) {
        var assetId = $("#asset_id");
        switch (type) {
            case "account_rs":
                return LRS.accountRS;
                break;
            case "message_link":
                return document.URL.replace(/#.*$/, "") + "#message:" + encodeURIComponent(LRS.accountRS);
                break;
            case "send_link":
                return document.URL.replace(/#.*$/, "") + "#send:" + encodeURIComponent(LRS.accountRS);
                break;
            case "asset_id":
                return assetId.text();
                break;
    ...
    LRS.dataLoaded = function (data, noPageLoad) {
        var $el = $("#" + LRS.currentPage + "_contents");

        if ($el.length) {
            $el.empty().append(data);
        } else {
            $el = $("#" + LRS.currentPage + "_table");
            $el.find("tbody").empty().append(data);
        }

        LRS.dataLoadFinished($el);
    ...
    LRS.dataLoadFinished = function ($el, fadeIn) {
    ...
    LRS.createInfoTable = function (data, fixed) {
        var rows = "";
        for (var key in data) {
            ...
            var match = key.match(/(.*)(LQT|QNT|RS)$/);
            var type = "";
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                if ($.isArray(value)) {
                    value = LRS.formatQuantity(value[0], value[1]);
                } else {
                    value = LRS.formatQuantity(value, 0);
                }
            } else if (key == "price" || key == "total" || key == "amount" || key == "fee" || key == "refund" || key == "discount") {
                value = LRS.formatAmount(new BigInteger(String(value))) + " LRD";
            } else if (key == "sender" || key == "recipient" || key == "account" || key == "seller" || key == "buyer" || key == "lessee") {
                value = "<a href='#' data-user='" + String(value).escapeHTML() + "' class='show_account_modal_action'>" + LRS.getAccountTitle(value) + "</a>";
    ...
    LRS.formatStyledAmount = function (strAmount, round) {
        var amount = LRS.formatAmount(strAmount, round).split(".");
        if (amount.length == 2) {
    ...
    LRS.getUnconfirmedTransactionsFromCache = function (type, subtype, fields, single) {
        if (!LRS.unconfirmedTransactions.length) {
            return false;
        }
        ...
        for (var i = 0; i < LRS.unconfirmedTransactions.length; i++) {
            var unconfirmedTransaction = LRS.unconfirmedTransactions[i];
            ...
            if (fields) {
                for (var key in fields) {
                    ...
                    if (unconfirmedTransaction[key] == fields[key]) {
                        if (single) {
                            return LRS.completeUnconfirmedTransactionDetails(unconfirmedTransaction);
                        } else {
        ...
        if (single || unconfirmedTransactions.length == 0) {
            ...
            $.each(unconfirmedTransactions, function (key, val) {
                unconfirmedTransactions[key] = LRS.completeUnconfirmedTransactionDetails(val);
            });
    ...
    LRS.completeUnconfirmedTransactionDetails = function (unconfirmedTransaction) {
    ...
    LRS.hasTransactionUpdates = function (transactions) {
        return ((transactions && transactions.length) || LRS.unconfirmedTransactionsChange);
    };

    LRS.showMore = function ($el) {
        if (!$el) {
            $el = $("#" + LRS.currentPage + "_contents");
            if (!$el.length) {
                $el = $("#" + LRS.currentPage + "_table");
            }
        }
    ...
    LRS.showFullDescription = function ($el) {
        $el.addClass("open").removeClass("closed");
        $el.find(".description_toggle").text("Less...");
    };

    LRS.showPartialDescription = function ($el) {
    ...
    $("body").on(".description_toggle", "click", function (e) {
        ...
        if ($(this).closest(".description").hasClass("open")) {
            LRS.showPartialDescription();
        } else {
            LRS.showFullDescription();
        }
    });
    ...
    LRS.setCookie = function (name, value, days) {
        var expires;
        if (days) {
            var date = new Date();
    ...
    LRS.deleteCookie = function (name) {
        LRS.setCookie(name, "", -1);
    };

    LRS.translateServerError = function (response) {
        var match;
            ...
            case 3:
                ...
                if (match && match[1]) {
                    return $.t("error_not_specified", {
                        "name": LRS.getTranslatedFieldName(match[1]).toLowerCase()
                    }).capitalize();
                }
                ...
                if (match && match[1]) {
                    ...
                    for (var i=0; i<fieldNames.length; i++) {
                        translatedFieldNames.push(LRS.getTranslatedFieldName(fieldNames[i]));
                    }
    ...
    LRS.getTranslatedFieldName = function (name) {
        var nameKey = String(name).replace(/LQT|QNT|RS$/, "").replace(/\s+/g, "").replace(/([A-Z])/g, function ($1) {
            return "_" + $1.toLowerCase();
        });
    ...
    LRS.isControlKey = function (charCode) {
        return !(charCode >= 32 || charCode == 10 || charCode == 13);
    };

    LRS.validateDecimals = function (maxFractionLength, charCode, val, e) {
        ...
        if (afterComma && afterComma[1].length > maxFractionLength) {
            var selectedText = LRS.getSelectedText();
    ...
    LRS.getUrlParameter = function (sParam) {
    ...
    LRS.getTransactionStatusIcon = function (phasedEntity) {
        var statusIcon;
