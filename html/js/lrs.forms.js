...
var LRS = (function(LRS, $) {
	LRS.confirmedFormWarning = false;
	...
	$(".modal form input").keydown(function(e) {
		if (e.which == "13") {
			e.preventDefault();
			if (LRS.settings["submit_on_enter"] && e.target.type != "textarea") {
				$(this).submit();
			} else {
				return false;
			}
	...
	$(".modal button.btn-primary:not([data-dismiss=modal]):not([data-ignore=true])").click(function() {
		LRS.submitForm($(this).closest(".modal"), $(this));
	});
	...
	LRS.addMessageData = function(data, requestType) {
			...
			if (data.encrypt_message) {
				try {
                    ...
                    if (data.doNotSign) {
                        ...
                        encrypted = LRS.encryptNote(data.message, options, data.secretPhrase);
				...
				if (!data.permanent_message && converters.stringToByteArray(data.message).length >= LRS.constants.MIN_PRUNABLE_MESSAGE_LENGTH) {
		...
		if (data.add_note_to_self && data.note_to_self) {
			try {
				if (data.doNotSign) {
                    ...
                    encrypted = LRS.encryptNote(data.note_to_self, {
                        "publicKey": converters.hexStringToByteArray(LRS.generatePublicKey(data.secretPhrase))
                    }, data.secretPhrase);
	...
	LRS.submitForm = function($modal, $btn) {
		if (!$btn) {
		...
		var formFunction = LRS["forms"][requestType];
		var formErrorFunction = LRS["forms"][requestType + "Error"];
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("LRD", "").replace("LQT", "").capitalize();
				var value = $(this).val();

				if ($(this).hasAttr("max")) {
					if (!/^[\-\d\.]+$/.test(value)) {
						error = $.t("error_not_a_number", {
							"field": LRS.getTranslatedFieldName(name).toLowerCase()
						}).capitalize();
					} else {
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				LRS.unlockForm($modal, $btn);
				return;
			} else if (!/^LRD\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^LRD\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
		...
		try {
			data = LRS.addMessageData(data, requestType);
		} catch (err) {
        ...
        if ("secretPhrase" in data && !data.secretPhrase.length && !LRS.rememberPassword) {
		...
		if (!LRS.showedFormWarning) {
			if ("amountLRD" in data && LRS.settings["amount_warning"] && LRS.settings["amount_warning"] != "0") {
				try {
					var amountLQT = LRS.convertToLQT(data.amountLRD);
				} catch (err) {
				...
				if (new BigInteger(amountLQT).compareTo(new BigInteger(LRS.settings["amount_warning"])) > 0) {
					LRS.showedFormWarning = true;
					$form.find(".error_message").html($.t("error_max_amount_warning", {
						"lrd": LRS.formatAmount(LRS.settings["amount_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			if ("feeLRD" in data && LRS.settings["fee_warning"] && LRS.settings["fee_warning"] != "0") {
				try {
					var feeLQT = LRS.convertToLQT(data.feeLRD);
				} catch (err) {
				...
				if (new BigInteger(feeLQT).compareTo(new BigInteger(LRS.settings["fee_warning"])) > 0) {
					LRS.showedFormWarning = true;
					$form.find(".error_message").html($.t("error_max_fee_warning", {
						"lrd": LRS.formatAmount(LRS.settings["fee_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			var convertLRDFields = ["phasingQuorumLRD", "phasingMinBalanceLRD"];
			$.each(convertLRDFields, function(key, field) {
				if (field in data) {
					try {
						LRS.convertToLQT(data[field]);
					} catch (err) {
		...
		LRS.sendRequest(requestType, data, function(response) {
			...
			if (response.fullHash) {
				...
				formCompleteFunction = LRS["forms"][originalRequestType + "Complete"];
				...
				if (requestType != "parseTransaction" && requestType != "calculateFullHash") {
					if (typeof formCompleteFunction == "function") {
						...
						LRS.addUnconfirmedTransaction(response.transaction);
					}
				} else {
					if (typeof formCompleteFunction == "function") {
						data.requestType = requestType;
	...
	LRS.unlockForm = function($modal, $btn, hide) {
