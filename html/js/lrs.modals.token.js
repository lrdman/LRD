...
var LRS = (function(LRS, $, undefined) {
	...
	LRS.forms.generateToken = function($modal) {
	...
	LRS.forms.generateTokenComplete = function(response, data) {
	...
	LRS.forms.generateTokenError = function() {
		$("#generate_token_output").hide();
	}

	LRS.forms.decodeTokenComplete = function(response, data) {
		$("#token_modal").find(".error_message").hide();

		if (response.valid) {
			$("#decode_token_output").html($.t("success_valid_token", {
				"account_link": LRS.getAccountLink(response, "account"),
				"timestamp": LRS.formatTimestamp(response.timestamp)
	...
	LRS.forms.decodeTokenError = function() {
