...
var LRS = (function(LRS, $, undefined) {
	LRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "LRD-____-____-____-_____") {
				LRS.checkRecipient(value, modal);
			} else {
	...
	$(".modal").on("click", "span.recipient_selector button, span.plain_adress_selector button", function(e) {
		if (!Object.keys(LRS.contacts).length) {
			e.preventDefault();
			e.stopPropagation();
			return;
		}
		...
		for (var accountId in LRS.contacts) {
			$list.append("<li><a href='#' data-contact-id='" + accountId + "' data-contact='" + String(LRS.contacts[accountId].name).escapeHTML() + "'>" + String(LRS.contacts[accountId].name).escapeHTML() + "</a></li>");
		}
	});
	...
	$(".modal").on("keyup blur show", ".plain_adress_selector_input", function(e) {
		...
		if (LRS.contacts[currentValue]) {
			var contactInfo = LRS.contacts[currentValue]['name'];
		} else {
			var contactInfo = " ";
		}
	...
	LRS.forms.sendMoneyComplete = function(response, data) {
		if (!(data["_extra"] && data["_extra"].convertedAccount) && !(data.recipient in LRS.contacts)) {
			$.growl($.t("success_send_money") + " <a href='#' data-account='" + LRS.getAccountFormatted(data, "recipient") + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>" + $.t("add_recipient_to_contacts_q") + "</a>", {
				"type": "success"
			});
		} else {
	...
	LRS.sendMoneyShowAccountInformation = function(accountId) {
		LRS.getAccountError(accountId, function(response) {
	...
	LRS.getAccountError = function(accountId, callback) {
			...
			if (response.publicKey) {
				if (response.name){
					callback({
						...
						"message": $.t("recipient_info_with_name", {
							"name" : response.name,
							"lrd": LRS.formatAmount(response.unconfirmedBalanceLQT, false, true)
						}),
						"account": response
					});
				}
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(LRD\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						});
	...
	LRS.checkRecipient = function(account, modal) {
		...
		if (/^(LRD\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
			var address = new LrdAddress();

			if (address.set(account)) {
				LRS.getAccountError(account, function(response) {
					if (response.noPublicKey && account!=LRS.accountRS) {
				...
				if (address.guess.length == 1) {
					callout.removeClass(classes).addClass("callout-danger").html($.t("recipient_malformed_suggestion", {
						"recipient": "<span class='malformed_address' data-address='" + String(address.guess[0]).escapeHTML() + "' onclick='LRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[0], account) + "</span>"
					})).show();
					...
					for (var i = 0; i < address.guess.length; i++) {
						html += "<li><span clas='malformed_address' data-address='" + String(address.guess[i]).escapeHTML() + "' onclick='LRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[i], account) + "</span></li>";
					}
			...
			if (LRS.databaseSupport && account.charAt(0) != '@') {
				LRS.database.select("contacts", [{
					"name": account
				}], function(error, contact) {
					if (!error && contact.length) {
						contact = contact[0];
						LRS.getAccountError(contact.accountRS, function(response) {
							if (response.noPublicKey && account!=LRS.account) {
								modal.find(".recipient_public_key").show();
							} else {
								modal.find("input[name=recipientPublicKey]").val("");
								modal.find(".recipient_public_key").hide();
							...
							callout.removeClass(classes).addClass("callout-" + response.type).html($.t("contact_account_link", {
								"account_id": LRS.getAccountFormatted(contact, "account")
						...
						LRS.checkRecipientAlias(account, modal);
					} else {
						callout.removeClass(classes).addClass("callout-danger").html($.t("recipient_malformed")).show();
					}
				});
	...
	LRS.checkRecipientAlias = function(account, modal) {
		var classes = "callout-info callout-danger callout-warning";
		var callout = modal.find(".account_info").first();
		var accountInputField = modal.find("input[name=converted_account_id]");
			...
			if (response.errorCode) {
				...
				if (response.aliasURI) {
					...
					if (match && match[1]) {
						...
						LRS.getAccountError(match[1], function(response) {
							...
							callout.removeClass(classes).addClass("callout-" + response.type).html($.t("alias_account_link", {
								...
								"timestamp": LRS.formatTimestamp(timestamp)
							})).show();
